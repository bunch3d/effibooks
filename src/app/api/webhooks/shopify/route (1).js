...
async function handleOrderCreated(supabase, shop, order) {
  const { error } = await supabase
    .from('order_snapshots')  // The table created in SCHEMA.sql Block 3
    .upsert(
      {
...
async function handleOrderUpdated(supabase, shop, order) {
  ...
  if (error) {
    console.error('[Webhook] Failed to update order in order_snapshots:', error.message);
    throw error;
  }

  console.info(`[Webhook] ✓ Order ${order.name} updated for ${shop}`);
}
